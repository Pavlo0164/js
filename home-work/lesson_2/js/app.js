...
productPriceOne = parseFloat(
  prompt("Введіть вартість одиниці першого товару", "0")
);
if (isNaN(productPriceOne)) {
  alert("Вартість не коректна");
} else {
  productNumberOne = parseInt(
    prompt("Введіть кількість одиниць першого товару", "0")
  );
  if (isNaN(productNumberOne)) {
    alert("Кількість не коректна");
  } else {
    productPriceTwo = parseFloat(
      prompt("Введіть вартість одиниці другого товару", "0")
    );
    if (isNaN(productPriceTwo)) {
      alert("Вартість не коректна");
    } else {
      productNumberTwo = parseInt(
        prompt("Введіть кількість одиниць другого товару", "0")
      );
      if (isNaN(productNumberTwo)) {
        alert("Кількість не коректна");
      } else {
        productPriceThree = parseFloat(
          prompt("Введіть вартість одиниці третього товару товару", "0")
        );
        if (isNaN(productPriceThree)) {
          alert("Вартість не коректна");
        } else {
          productNumberThree = parseInt(
            prompt("Введіть кількість одиниць третього товару", "0")
          );
